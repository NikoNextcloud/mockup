...
import { generateMockup, generateAsset, generateRealtimeComposite, generateMockupReview } from './services/geminiService';
...
const renderMarkdownSimple = (text: string) => {
  ...
  return text.split('\n').map((line, idx) => {
    ...
    if (trimmed.startsWith('###')) {
      return <h3 key={idx} className="text-lg font-bold text-indigo-400 mt-4 mb-2">{trimmed.replace(/^###\s*/, '')}</h3>;
    }
    if (trimmed.startsWith('##')) {
      return <h2 key={idx} className="text-xl font-bold text-indigo-300 mt-5 mb-2">{trimmed.replace(/^##\s*/, '')}</h2>;
    }
    if (trimmed.startsWith('#')) {
      return <h1 key={idx} className="text-2xl font-extrabold text-indigo-500 mt-6 mb-3">{trimmed.replace(/^#\s*/, '')}</h1>;
    }
    if (trimmed.startsWith('-') || trimmed.startsWith('*')) {
      const content = trimmed.replace(/^[-*]\s*/, '');
      // Check for inline bold text e.g. **Bold**: normal
      const boldMatch = content.match(/^\*\*(.*?)\*\*:(.*)$/);
      if (boldMatch) {
        return (
          <li key={idx} className="list-disc ml-5 text-zinc-300 text-sm mb-1.5 leading-relaxed">
            <strong className="text-indigo-200 font-semibold">{boldMatch[1]}:</strong>{boldMatch[2]}
          </li>
        );
      }
      return <li key={idx} className="list-disc ml-5 text-zinc-300 text-sm mb-1.5 leading-relaxed">{content}</li>;
    }
    if (trimmed === '') {
      return <div key={idx} className="h-2"></div>;
    }
    // Check for standard inline bolding **word**
    ...
    return <p key={idx} className="text-zinc-300 text-sm leading-relaxed mb-2">{trimmed}</p>;
  });
};
...
export default function App() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Get names of first logo and product for AI text generation
      const firstLogoName = layers[0]?.asset?.name || "Custom Logo";
      const productTitle = product.name || "Merchandise Blank";

      // Show message that we are writing AI description
      setLoading({ isGenerating: true, message: 'Drafting AI product copy and design review...' });
      const aiDesc = await generateMockupReview(productTitle, firstLogoName, currentPrompt);
  ...
  return (
    <div className="min-h-screen bg-black text-zinc-100 font-sans flex overflow-hidden relative">
      ...
      {selectedMockup && (
        ...
        >
          <div 
            className="relative max-w-5xl w-full h-[90vh] md:h-[80vh] bg-zinc-950 border border-zinc-800 rounded-2xl overflow-hidden flex flex-col md:flex-row shadow-2xl"
            onClick={e => e.stopPropagation()}
          >
            {/* Close Button */}
            <button 
              onClick={() => setSelectedMockup(null)}
              className="absolute top-4 right-4 p-2 bg-zinc-900/80 hover:bg-zinc-800 text-white rounded-full transition-colors z-50 border border-zinc-700 shadow-md"
            >
              <X size={20} />
            </button>

            {/* Left side: Mockup image display */}
            <div className="flex-1 bg-zinc-900 flex flex-col items-center justify-center p-6 relative border-b md:border-b-0 md:border-r border-zinc-800 h-[45%] md:h-full">
            ...
            {/* Right side: AI marketing copy & details */}
            <div className="w-full md:w-[400px] bg-zinc-950 p-6 md:p-8 flex flex-col justify-between overflow-y-auto h-[55%] md:h-full">
              <div className="space-y-6">
                <div>
                  <span className="text-xs font-mono font-bold tracking-widest text-indigo-400 uppercase bg-indigo-500/10 px-2 py-1 rounded">AI Brand Studio</span>
                  <h2 className="text-2xl font-black text-white mt-3 tracking-tight">Merchandise Review</h2>
                  <p className="text-xs text-zinc-500 mt-1">Generated {new Date(selectedMockup.createdAt).toLocaleDateString()}</p>
                </div>

                <div className="border-t border-zinc-800 pt-4 overflow-y-auto max-h-[30vh] md:max-h-[45vh] pr-2 scrollbar-thin">
                  {selectedMockup.aiDescription ? (
                    <div className="space-y-1">
                      {renderMarkdownSimple(selectedMockup.aiDescription)}
                    </div>
                  ) : (
                    <div className="text-zinc-500 text-sm space-y-2">
                      <p>No AI description generated yet.</p>
                      <p className="text-xs text-zinc-600">Connect a free Gemini API Key to automatically craft product reviews and marketing taglines!</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="border-t border-zinc-800 pt-6 flex flex-col sm:flex-row gap-3">
                <a 
                  ...
                  className="flex-1"
                >
                  <Button className="w-full flex items-center justify-center gap-2" icon={<Download size={16} />}>
                    Download Image
                  </Button>
                </a>
                <Button variant="outline" onClick={() => setSelectedMockup(null)}>
                  Close Studio
                </Button>
              </div>
            </div>
